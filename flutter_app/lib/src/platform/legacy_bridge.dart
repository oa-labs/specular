...
class LegacyBridge {
  ...
  Future<LegacyState> readState() async {
    ...
    final raw = Map<Object?, Object?>.from(
      await _channel.invokeMethod<Map<Object?, Object?>>('readLegacyState') ?? const {},
    );
    return LegacyState(
      ...
      deselectedFolders: List<String>.from(raw['deselectedFolders'] as List? ?? const []),
      secrets: Map<String, Object?>.from(raw['secrets'] as Map? ?? const {}),
    );
  }

  Future<void> migrateSecrets(LegacyState state, FlutterSecureStorage storage) async {
    if (await storage.read(key: _completeKey) == 'true') return;
    for (final entry in state.secrets.entries) {
      final value = entry.value;
      if (value is String && value.isNotEmpty) await storage.write(key: entry.key, value: value);
      if (value is bool) await storage.write(key: entry.key, value: value.toString());
    }
    if (state.deselectedFolders.isNotEmpty) {
      await storage.write(key: 'deselected_folders', value: state.deselectedFolders.join('\u001f'));
    }
    await storage.write(key: _completeKey, value: 'true');
    if (Platform.isAndroid) await _channel.invokeMethod<void>('markLegacyMigrationComplete');
  }
}
