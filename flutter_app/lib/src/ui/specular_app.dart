...
final noteRepositoryProvider = Provider<NoteRepository>((_) => throw UnimplementedError());
final secureStorageProvider = Provider<FlutterSecureStorage>((_) => throw UnimplementedError());
final syncEngineProvider = Provider<GitHubSyncEngine>(
  (ref) => GitHubSyncEngine(ref.watch(noteRepositoryProvider), ref.watch(secureStorageProvider)),
);
final voiceServiceProvider = Provider<VoiceService>((ref) => VoiceService(ref.watch(secureStorageProvider)));
final notesProvider = StreamProvider<List<Note>>((ref) => ref.watch(noteRepositoryProvider).watchNotes());
final todosProvider = StreamProvider<List<TodoItem>>((ref) => ref.watch(noteRepositoryProvider).watchTodos());
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final router = GoRouter(
      routes: [
        ...
        GoRoute(
          path: '/note/:id',
          builder: (_, state) => NoteDetailScreen(id: state.pathParameters['id']!),
        ),
        GoRoute(
          path: '/editor/:id',
